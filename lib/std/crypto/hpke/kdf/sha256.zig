...
fn labeledExtractImpl(
    ...
    const prk = HkdfSha256.extract(salt_slice, buf[0..len]);
    @memcpy(out[0..32], &prk);
    out_len.* = 32;
}
...
fn labeledExpandImpl(
    ...
    var buf: [512]u8 = undefined;
    const len = buildLabeled(&parts, &buf);
    var prk_arr: [32]u8 = undefined;
    @memcpy(&prk_arr, prk[0..32]);
    HkdfSha256.expand(out, buf[0..len], prk_arr);
}

pub const HkdfSha256Vtable = KdfVtable{
    .hash_len = 32,
