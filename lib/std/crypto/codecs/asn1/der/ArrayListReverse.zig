...
pub fn prependByte(self: *ArrayListReverse, byte: u8) Error!void {
    return self.prependSlice(&.{byte});
}

pub fn prependInt(self: *ArrayListReverse, comptime T: type, value: T, endian: std.builtin.Endian) Error!void {
    var buf: [@divExact(@typeInfo(T).int.bits, 8)]u8 = undefined;
    std.mem.writeInt(T, &buf, value, endian);
    return self.prependSlice(&buf);
}
