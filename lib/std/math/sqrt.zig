...
pub fn sqrt(x: anytype) Sqrt(@TypeOf(x)) {
    ...
    switch (@typeInfo(T)) {
        ...
        .comptime_int => comptime {
            if (x < 0) @compileError("sqrt on negative number");
            return sqrtInt(math.IntFittingRange(0, x), x);
        },
        .int => |IntType| switch (IntType.signedness) {
            .signed => @compileError("sqrt not implemented for signed integers"),
            .unsigned => return sqrtInt(T, x),
        },
        else => @compileError("sqrt not implemented for " ++ @typeName(T)),
    }
}

fn sqrtInt(comptime T: type, value: T) Sqrt(T) {
    if (@typeInfo(T).int.bits <= 2) {
        return if (value == 0) 0 else 1; // shortcut for small number of bits to simplify general case
    } else {
        const bits = @typeInfo(T).int.bits;
        const minustwo = @as(T, 0) -% 2; // unsigned int cannot represent -2
        var op = value;
        var res: T = 0;
...
test sqrtInt {
    ...
    try expect(sqrtInt(u32, 8) == 2);
    try expect(sqrtInt(u32, 9) == 3);
    try expect(sqrtInt(u32, 10) == 3);

    try expect(sqrtInt(u0, 0) == 0);
    try expect(sqrtInt(u1, 1) == 1);
    try expect(sqrtInt(u2, 3) == 1);
