...
pub fn frexp(x: anytype) Frexp(@TypeOf(x)) {
    ...
    const bits, const F = switch (@typeInfo(T)) {
    ...
    result.significand = @as(F, @bitCast(v));
    return result;
}
...
fn FrexpTests(comptime Float: type) type {
    return struct {
        ...
        test "inf" {
            ...
            const r1 = frexp(math.inf(T));
            try expectEqual(0, r1.exponent);
            try expect(math.isPositiveInf(r1.significand));

            const r2 = frexp(-math.inf(T));
            try expectEqual(0, r2.exponent);
            try expect(math.isNegativeInf(r2.significand));
        }
        test "nan" {
            if (T == comptime_float) return;
...
test frexp {
    ...
    inline for ([_]type{ f16, f32, f64, f80, f128, comptime_float }) |T| {
        ...
        var result: Frexp(T) = frexp(math.inf(T));
