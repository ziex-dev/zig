...
pub const Mutable = struct {
    ...
    pub fn mulNoAlias(rma: *Mutable, a: Const, b: Const, allocator: ?Allocator) void {
        ...
        raw.llmulacc(.add, allocator, rma.limbs, a.limbs, b.limbs);
    ...
    pub fn mulWrapNoAlias(
        ...
        raw.llmulacc(.add, allocator, rma.limbs, a_limbs, b_limbs);
    ...
    pub fn sqrNoAlias(rma: *Mutable, a: Const, opt_allocator: ?Allocator) void {
        ...
        raw.llsquareBasecase(rma.limbs, a.limbs);
    ...
    pub fn shiftLeft(r: *Mutable, a: Const, shift: usize) void {
        const new_len = raw.llshl(r.limbs, a.limbs, shift);
        r.normalize(new_len);
        r.positive = a.positive;
    }
    ...
    pub fn shiftRight(r: *Mutable, a: Const, shift: usize) void {
        ...
        const new_len = raw.llshr(r.limbs, a.limbs, shift);
    ...
    pub fn bitOr(r: *Mutable, a: Const, b: Const) void {
        ...
        if (a.limbs.len >= b.limbs.len) {
            r.positive = raw.llsignedor(r.limbs, a.limbs, a.positive, b.limbs, b.positive);
            r.normalize(if (b.positive) a.limbs.len else b.limbs.len);
        } else {
            r.positive = raw.llsignedor(r.limbs, b.limbs, b.positive, a.limbs, a.positive);
    ...
    pub fn bitAnd(r: *Mutable, a: Const, b: Const) void {
        ...
        if (a.limbs.len >= b.limbs.len) {
            r.positive = raw.llsignedand(r.limbs, a.limbs, a.positive, b.limbs, b.positive);
            r.normalize(if (b.positive) b.limbs.len else if (a.positive) a.limbs.len else a.limbs.len + 1);
        } else {
            r.positive = raw.llsignedand(r.limbs, b.limbs, b.positive, a.limbs, a.positive);
    ...
    pub fn bitXor(r: *Mutable, a: Const, b: Const) void {
        ...
        if (a.limbs.len > b.limbs.len) {
            r.positive = raw.llsignedxor(r.limbs, a.limbs, a.positive, b.limbs, b.positive);
            r.normalize(a.limbs.len + @intFromBool(a.positive != b.positive));
        } else {
            r.positive = raw.llsignedxor(r.limbs, b.limbs, b.positive, a.limbs, a.positive);
    ...
    pub fn pow(r: *Mutable, a: Const, b: u32, limbs_buffer: []Limb) void {
        ...
        raw.llpow(r.limbs, a.limbs, b, limbs_buffer);
    ...
    fn div(q: *Mutable, r: *Mutable, x: *Mutable, y: *Mutable) void {
        ...
        if (y.len - xy_trailing == 1) {
            ...
            if (divisor < maxInt(HalfLimb)) {
                raw.lldiv0p5(q.limbs, &r.limbs[0], x.limbs[xy_trailing..x.len], @as(HalfLimb, @intCast(divisor)));
            } else {
                raw.lldiv1(q.limbs, &r.limbs[0], x.limbs[xy_trailing..x.len], divisor);
            }
    ...
    fn divmod(
        ...
        while (i >= t + 1) : (i -= 1) {
            ...
            while (true) {
                ...
                tmp1[2] = raw.addMulLimbWithCarry(0, y0, q.limbs[k], &tmp1[0]);
                tmp1[1] = raw.addMulLimbWithCarry(0, y1, q.limbs[k], &tmp1[0]);
            ...
            // to x[i - t - 1].
            const underflow = raw.llmulLimb(.sub, x.limbs[k..x.len], y.limbs[0..y.len], q.limbs[k]);
            ...
            if (underflow) {
                ...
                raw.llaccum(.add, x.limbs[k..x.len], y.limbs[0..y.len]);
                q.limbs[k] -= 1;
            }
        }
    ...
    pub fn truncate(r: *Mutable, a: Const, signedness: Signedness, bit_count: usize) void {
        ...
        const abs_trunc_a: Const = .{
            .positive = true,
            .limbs = a.limbs[0..raw.llnormalize(a.limbs[0..@min(a.limbs.len, max_limbs)])],
        };
    ...
    pub fn normalize(r: *Mutable, length: usize) void {
        r.len = raw.llnormalize(r.limbs[0..length]);
    }
...
pub const Const = struct {
    ...
    pub fn toFloat(self: Const, comptime Float: type, round: Round) struct { Float, Exactness } {
        ...
        const normalized_abs: Const = .{
            .limbs = self.limbs[0..raw.llnormalize(self.limbs)],
            .positive = true,
        };
