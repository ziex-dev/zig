...
//! allocator is used.  reset() is provided, but will NOT attempt to reset()
//! the secondary allocator (e.g., an ArenaAllocator) - caller must do that.
...
/// Resets the primary (FixedBufferAllocator) ONLY -
/// will NOT reset the secondary allocator - that is the caller's responsibility
...
test StackFirstAllocator {
    ...
    sfa.reset(); // NOTE: will NOT reset secondary allocator
