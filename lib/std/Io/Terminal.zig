...
pub const Mode = union(enum) {
    ...
    pub fn setColor(mode: Mode, writer: *std.Io.Writer, color: Color) Mode.SetColorError!void {
        switch (mode) {
            .no_color => return,
            .escape_codes => {
...
/// Equivalent to calling `t.mode.setColor(t.writer, color)`.
pub fn setColor(t: Terminal, color: Color) SetColorError!void {
    try t.mode.setColor(t.writer, color);
}

test "write terminal message with escape codes" {
    var terminal_writer: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer terminal_writer.deinit();

    const terminal: Terminal = .{
        .writer = &terminal_writer.writer,
        .mode = .escape_codes,
    };

    try terminal.setColor(.red);
    try terminal.setColor(.bold);
    try terminal.writer.writeAll("Hello ");
    try terminal.setColor(.reset);

    try terminal.setColor(.bright_green);
    try terminal.setColor(.dim);
    try terminal.writer.writeAll("there.");
    try terminal.setColor(.reset);

    try std.testing.expectEqualStrings(
        "\x1b[31m\x1b[1mHello \x1b[0m\x1b[92m\x1b[2mthere.\x1b[0m",
        terminal_writer.written(),
    );
}

test "write terminal message with no color" {
    var terminal_writer: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer terminal_writer.deinit();
