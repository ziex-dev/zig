...
pub const ResolvConf = struct {
    ...
    pub fn init(allocator: Allocator) ResolvConf {
        return .{
            .allocator = allocator,
    ...
    pub fn parseFromAbsolutePath(rc: *ResolvConf, io: Io, path: []const u8) !void {
        const file = Io.Dir.openFileAbsolute(io, path, .{}) catch |err| switch (err) {
            error.FileNotFound,
            error.NotDir,
            error.AccessDenied,
            => return addNumeric(rc, io, "127.0.0.1", 53),
...
test ResolvConf {
...

    {
        ...
        var rc: ResolvConf = .init(allocator);
        defer rc.deinit();

        try rc.parse(io, &reader);
...

    {
        const input =
            ...
            \\nameserver 
            \\
            \\options timeout:1 attempts:1 rotate ndots:0
            \\options
            \\options 
            \\search foo.test
            \\domain bar.test
        ...
        try rc.parse(io, &reader);
        try std.testing.expectEqual(2, rc.nameservers().len);
        try std.testing.expectEqual(1, rc.attempts);
        try std.testing.expectEqual(0, rc.ndots);
        try std.testing.expectEqual(1, rc.timeout_seconds);
        try std.testing.expectEqualSlices(u8, "bar.test", rc.search.?);
    }
}
