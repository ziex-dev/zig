...
fn filePathKind(ev: *Evented, dir: Dir, sub_path: []const u8) !File.Kind {
    ...
    while (true) {
        ...
        thread.enqueue().* = .{
            ...
            .addr = @intFromPtr(sub_path_posix.ptr),
            .len = @bitCast(linux.Statx.Mask{ .TYPE = true }),
...
fn fileLength(userdata: ?*anyopaque, file: File) File.LengthError!u64 {
    ...
    while (true) {
        ...
        thread.enqueue().* = .{
            ...
            .off = @intFromPtr(&statx_buf),
            .addr = @intFromPtr(""),
            .len = @bitCast(linux.Statx.Mask{ .SIZE = true }),
...
fn netBindIp(
    ...
    if (options.allow_broadcast) try ev.setsockopt(&maybe_sync.cancel_region, socket_fd, @intFromEnum(linux.SOL.SOCKET), @intFromEnum(linux.SO.BROADCAST), 1);
