(function() {
    ...
    function onSearchKeyDown(ev) {
      ...
      if (ev.shiftKey || ev.ctrlKey || ev.altKey || ev.isComposing) return;
    ...
    function onWindowKeyDown(ev) {
        if (ev.ctrlKey || ev.altKey || ev.isComposing) return;
        ...
        switch (ev.key) {
            ...
            case "?":
                break;
            default:
                // As a fallback, to ensure non-Latin keyboard layouts can access shortcuts,
                // also accept the keys that physically correspond to a US QWERTY layout
                switch (ev.code) {
                    case "KeyS":
                        key = "s";
                        break;
