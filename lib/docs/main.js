(function() {
    ...
    const domHdrNameSec = document.getElementById("hdrNameSec");
    const domHdrName = document.getElementById("hdrName");
    const domHdrKind = document.getElementById("hdrKind");
    ...
    const domSrcLink = document.getElementById("srcLink");
    ...
    function render() {
        ...
        domHdrNameSec.classList.add("hidden");
        domSrcLink.classList.add("hidden");
    ...
    function renderDeclHeading(decl_index) {
      ...
      hdrName.innerText = unwrapString(wasm_exports.decl_name(decl_index));
      hdrKind.innerText = unwrapString(wasm_exports.decl_category_name(decl_index));
      srcLink.setAttribute('href', curNav.viewSourceHash);

      domHdrNameSec.classList.remove("hidden");
      domSrcLink.classList.remove("hidden");
    ...
    function renderParams(decl_index) {
      ...
      if (params.length !== 0) {
          resizeDomList(domListParams, params.length, '<tbody></tbody>');
    ...
    function renderNamespace(base_decl, members, fields) {
      ...
      if (fields.length !== 0) {
          resizeDomList(domListFields, fields.length, '<tbody></tbody>');
          for (let i = 0; i < fields.length; i += 1) {
              const trDom = domListFields.children[i];
              trDom.innerHTML = unwrapString(wasm_exports.decl_field_html(base_decl, fields[i]));
          }
      ...
      const tbodyDom = domListValues.children[0];
          resizeDomList(tbodyDom, valsList.length,
              '<tr><td class="item-name"><a href="#"></a></td><td class="item-type"></td><td></td></tr>');
          for (let i = 0; i < valsList.length; i += 1) {
              const trDom = tbodyDom.children[i];
              const tdName = trDom.children[0];
              const tdNameA = tdName.children[0];
              const tdType = trDom.children[1];
              const tdDesc = trDom.children[2];
