...
/// Parses and walks any file that has not been parsed yet. Decls are only
/// discovered by walking, so anything that scans the global `decls` list for
/// completeness (such as search) must call this first.
pub fn ensureAllFilesParsed() void {
    for (0..files.entries.len) |i| {
        const file_index: File.Index = @enumFromInt(i);
        _ = file_index.get_ast();
    }
}
