...
static inline int unsupported_api_check(const char *osname)
{
#ifdef __LITEOS_A__
	if (!strcmp(osname, LITEOS_A)) {
		return 1;
	}
...
#define UNSUPPORTED_API_VOID(osname)                                   \
	if (unsupported_api_check(osname) > 0) {                           \
		fprintf(stderr, "[ERR]Unsupported API %s\n", __FUNCTION__);    \
	}
#endif
