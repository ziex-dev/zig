...
const Args = struct {
    ...
    pub const @"--help": std.cli.Help(Args) = .{
        .args = .{
            ...
            .vendor = .{
                .display = "(musl|glibc|freebsd|netbsd|openbsd)",
                .description = "Libc vendor",
            },
            .@"--search-path" = .{
                .display = "<path>",
...
pub fn main(init: std.process.Init, args: Args) !void {
    ...
    const generic_name = try std.fmt.allocPrint(arena, "generic-{s}", .{@tagName(args.vendor)});
    const libc_targets = switch (args.vendor) {
        .glibc => &glibc_targets,
        .musl => &musl_targets,
    ...
    for (libc_targets) |libc_target| {
        const libc_dir = switch (args.vendor) {
        ...
        search: for (args.@"--search-path") |search_path| {
            const sub_path = switch (args.vendor) {
                .glibc,
                .freebsd,
                .netbsd,
                .openbsd,
    ...
    while (path_it.next()) |path_kv| {
        ...
        while (hash_it.next()) |hash_kv| {
            ...
            const full_path = try Dir.path.join(arena, &[_][]const u8{ args.out_dir, dest_target, path_kv.key_ptr.* });
            try Dir.cwd().createDirPath(io, Dir.path.dirname(full_path).?);
