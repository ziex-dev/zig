...
const usage =
    \\usage: update_freebsd_libc -- path/downloaded/freebsd-src path/repo/zig
    \\
...
const command: std.cli.Command = .{
    ...
    .help = usage,
    .positional_args = &.{
        .init([:0]const u8, .{ .name = "freebsd_src_path" }),
...
pub fn main(init: std.process.Init) !void {
    ...
    const freebsd_src_path = parsed.kind.args.freebsd_src_path;
