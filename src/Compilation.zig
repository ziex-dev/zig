...
pub fn update(comp: *Compilation, main_progress_node: std.Progress.Node) UpdateError!void {
    ...
    switch (comp.cache_use) {
        ...
        .whole => |whole| {
            ...
            // observed in one other niche environment (windows-hosted 9p
            // from WSL).
            const need_writable_dance = dance: {
                // The workaround is only guaranteed required on windows, so try
                // just doing the rename straight up first on other platforms.
                if (builtin.os.tag != .windows) fastpath: {
                ...
                log.debug("trying windows-style AccessDenied workaround\n", .{});

                const d = if (comp.bin_file) |f| f.closeBin() else false;
                renameTmpIntoCache(io, comp.dirs.local_cache, tmp_dir_sub_path, o_sub_path) catch |err| {
