...
pub fn loadInput(elf: *Elf, input: link.Input) link.Error!void {
    const diags = &elf.base.comp.link_diags;
...
fn loadInputInner(elf: *Elf, input: link.Input) Error!void {
    const comp = elf.base.comp;
...
fn loadArchive(elf: *Elf, path: std.Build.Cache.Path, fr: *Io.File.Reader) LoadParseInputError!void {
    const comp = elf.base.comp;
    const gpa = comp.gpa;
    const diags = &comp.link_diags;
    ...
    if (!std.mem.eql(u8, try r.take(std.elf.ARMAG.len), std.elf.ARMAG)) {
        return diags.failParse(path, "bad archive magic", .{});
    }
...
fn loadDso(elf: *Elf, path: std.Build.Cache.Path, fr: *Io.File.Reader) LoadParseInputError!void {
    const comp = elf.base.comp;
    const gpa = comp.gpa;
    const diags = &comp.link_diags;
    const r = &fr.interface;
...
fn checkInputIdent(
    ...
    path: std.Build.Cache.Path,
    r: *Io.Reader,
) error{ AlreadyReported, EndOfStream, ReadFailed }!void {
    const diags = &elf.base.comp.link_diags;
    ...
    if (!std.mem.eql(u8, &ident.magic, std.elf.MAGIC)) return diags.failParse(
        path,
        "bad ELF magic",
        .{},
    );
