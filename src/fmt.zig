...
pub fn run(gpa: Allocator, arena: Allocator, io: Io, args: []const []const u8) !void {
    ...
    if (stdin_flag) {
        ...
        var tree = std.zig.Ast.parsePackedAutofix(gpa, source_code, if (force_zon) .zon else .zig, true) catch |err| {
...
fn fmtPathFile(
    ...
    var tree = try std.zig.Ast.parsePackedAutofix(gpa, source_code, mode, true);
    defer tree.deinit(gpa);
