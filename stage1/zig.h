...
#elif defined(zig_msvc)
#define zig_linksection(name)                                                  \
  __pragma(section(name, read, write)) __declspec(allocate(name))
#define zig_linksection_fn(name)                                               \
  __pragma(section(name, read, execute)) __declspec(code_seg(name))
#else
...
#define zig_export(symbol, name)                                               \
  ;                                                                            \
  __pragma(comment(                                                            \
      linker, "/alternatename:" zig_mangle_c(name) "=" zig_mangle_c(symbol)))
...
#define zig_mangled_export(mangled, unmangled, symbol)                         \
  zig_export(unmangled, #mangled) zig_export(symbol, unmangled)
#else /* zig_msvc */
#define zig_mangled(mangled, unmangled) __asm(zig_mangle_c(unmangled))
#define zig_mangled_export(mangled, unmangled, symbol)                         \
  zig_mangled_final(mangled, unmangled) zig_export(symbol, unmangled)
#endif /* zig_msvc */

#if defined(zig_msvc)
#define zig_import(Type, fn_name, libc_name, sig_args, call_args)              \
  zig_extern Type fn_name sig_args;                                            \
  __pragma(comment(linker, "/alternatename:" zig_mangle_c(                     \
                               #fn_name) "=" zig_mangle_c(#libc_name)));
#define zig_import_builtin(Type, fn_name, libc_name, sig_args, call_args)      \
  zig_import(Type, fn_name, libc_name, sig_args, call_args)
#else /* zig_msvc */
#define zig_import(Type, fn_name, libc_name, sig_args, call_args)              \
  zig_extern Type fn_name sig_args __asm(zig_mangle_c(#libc_name));
#define zig_import_builtin(Type, fn_name, libc_name, sig_args, call_args)      \
  zig_extern Type libc_name sig_args;                                          \
  static inline Type fn_name sig_args { return libc_name call_args; }
#endif

#define zig_expand_import_0(Type, fn_name, libc_name, sig_args, call_args)     \
  zig_import(Type, fn_name, libc_name, sig_args, call_args)
#define zig_expand_import_1(Type, fn_name, libc_name, sig_args, call_args)     \
  zig_import_builtin(Type, fn_name, libc_name, sig_args, call_args)
...
#if SCHAR_MIN == ~0x7FFFFFFF && SCHAR_MAX == 0x7FFFFFFF &&                     \
    UCHAR_MAX == 0xFFFFFFFF
typedef unsigned char uint32_t;
typedef signed char int32_t;
#define INT32_C(c) c
#define UINT32_C(c) c##U
#elif SHRT_MIN == ~0x7FFFFFFF && SHRT_MAX == 0x7FFFFFFF &&                     \
    USHRT_MAX == 0xFFFFFFFF
...
#define INT32_C(c) c
#define UINT32_C(c) c##U
#elif LONG_MIN == ~0x7FFFFFFF && LONG_MAX == 0x7FFFFFFF &&                     \
    ULONG_MAX == 0xFFFFFFFF
typedef unsigned long uint32_t;
typedef signed long int32_t;
#define INT32_C(c) c##L
#define UINT32_C(c) c##LU
#elif LLONG_MIN == ~0x7FFFFFFF && LLONG_MAX == 0x7FFFFFFF &&                   \
    ULLONG_MAX == 0xFFFFFFFF
...
#if SCHAR_MIN == ~0x7FFFFFFFFFFFFFFF && SCHAR_MAX == 0x7FFFFFFFFFFFFFFF &&     \
    UCHAR_MAX == 0xFFFFFFFFFFFFFFFF
typedef unsigned char uint64_t;
typedef signed char int64_t;
#define INT64_C(c) c
#define UINT64_C(c) c##U
#elif SHRT_MIN == ~0x7FFFFFFFFFFFFFFF && SHRT_MAX == 0x7FFFFFFFFFFFFFFF &&     \
    USHRT_MAX == 0xFFFFFFFFFFFFFFFF
typedef unsigned short uint64_t;
typedef signed short int64_t;
#define INT64_C(c) c
#define UINT64_C(c) c##U
#elif INT_MIN == ~0x7FFFFFFFFFFFFFFF && INT_MAX == 0x7FFFFFFFFFFFFFFF &&       \
    UINT_MAX == 0xFFFFFFFFFFFFFFFF
typedef unsigned int uint64_t;
typedef signed int int64_t;
#define INT64_C(c) c
#define UINT64_C(c) c##U
#elif LONG_MIN == ~0x7FFFFFFFFFFFFFFF && LONG_MAX == 0x7FFFFFFFFFFFFFFF &&     \
    ULONG_MAX == 0xFFFFFFFFFFFFFFFF
typedef unsigned long uint64_t;
typedef signed long int64_t;
#define INT64_C(c) c##L
#define UINT64_C(c) c##LU
#elif LLONG_MIN == ~0x7FFFFFFFFFFFFFFF && LLONG_MAX == 0x7FFFFFFFFFFFFFFF &&   \
...
#define zig_intMin_i8 INT8_MIN
#define zig_intMax_i8 INT8_MAX
#define zig_intMin_u8 UINT8_C(0)
#define zig_intMax_u8 UINT8_MAX
#define zig_intMin_i16 INT16_MIN
#define zig_intMax_i16 INT16_MAX
#define zig_intMin_u16 UINT16_C(0)
#define zig_intMax_u16 UINT16_MAX
#define zig_intMin_i32 INT32_MIN
#define zig_intMax_i32 INT32_MAX
#define zig_intMin_u32 UINT32_C(0)
#define zig_intMax_u32 UINT32_MAX
#define zig_intMin_i64 INT64_MIN
#define zig_intMax_i64 INT64_MAX
#define zig_intMin_u64 UINT64_C(0)
#define zig_intMax_u64 UINT64_MAX

#define zig_intLimit(s, w, limit, bits)                                        \
  zig_shr_##s##w(zig_int##limit##_##s##w, w - (bits))
#define zig_intMin_i(w, bits) zig_intLimit(i, w, Min, bits)
#define zig_intMax_i(w, bits) zig_intLimit(i, w, Max, bits)
#define zig_intMin_u(w, bits) zig_intLimit(u, w, Min, bits)
#define zig_intMax_u(w, bits) zig_intLimit(u, w, Max, bits)
...
#define zig_int_helpers(w, PromotedUnsigned)                                   \
  zig_basic_operator(uint##w##_t, and_u##w, &) zig_basic_operator(             \
      int##w##_t, and_i##w, &) zig_basic_operator(uint##w##_t, or_u##w, |)     \
                  ...
                  zig_shift_operator(uint##w##_t, shl_u##w, <<)                \
                      zig_shift_operator(int##w##_t, shl_i##w, <<)             \
                          zig_shift_operator(uint##w##_t, shr_u##w, >>)        \
                                                                               \
                              static inline int##w##_t                         \
                              zig_shr_i##w(int##w##_t lhs, uint8_t rhs) {      \
    int##w##_t sign_mask =                                                     \
        lhs < INT##w##_C(0) ? -INT##w##_C(1) : INT##w##_C(0);                  \
    return ((lhs ^ sign_mask) >> rhs) ^ sign_mask;                             \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_not_u##w(uint##w##_t val, uint8_t bits) {      \
    return val ^ zig_intMax_u(w, bits);                                        \
  }                                                                            \
                                                                               \
  ...
  static inline int##w##_t zig_wrap_i##w(int##w##_t val, uint8_t bits) {       \
    return (val & UINT##w##_C(1) << (bits - UINT8_C(1))) != 0                  \
               ? val | zig_intMin_i(w, bits)                                   \
               : val & zig_intMax_i(w, bits);                                  \
  }                                                                            \
                                                                               \
  ...
  zig_basic_operator(uint##w##_t, div_floor_u##w, /)                           \
                                                                               \
      static inline int##w##_t                                                 \
      zig_div_floor_i##w(int##w##_t lhs, int##w##_t rhs) {                     \
    return lhs / rhs + (lhs % rhs != INT##w##_C(0)                             \
                            ? zig_shr_i##w(lhs ^ rhs, UINT8_C(w) - UINT8_C(1)) \
                            : INT##w##_C(0));                                  \
  }                                                                            \
                                                                               \
  zig_basic_operator(uint##w##_t, mod_u##w, %)                                 \
                                                                               \
      static inline int##w##_t                                                 \
      zig_mod_i##w(int##w##_t lhs, int##w##_t rhs) {                           \
    int##w##_t rem = lhs % rhs;                                                \
    return rem + (rem != INT##w##_C(0)                                         \
                      ? rhs & zig_shr_i##w(lhs ^ rhs, UINT8_C(w) - UINT8_C(1)) \
                      : INT##w##_C(0));                                        \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_shlw_u##w(uint##w##_t lhs, uint8_t rhs,        \
                                          uint8_t bits) {                      \
    return zig_wrap_u##w(zig_shl_u##w(lhs, rhs), bits);                        \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_shlw_i##w(int##w##_t lhs, uint8_t rhs,          \
                                         uint8_t bits) {                       \
    return zig_wrap_i##w((int##w##_t)zig_shl_u##w((uint##w##_t)lhs, rhs),      \
                         bits);                                                \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_addw_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    return zig_wrap_u##w(lhs + rhs, bits);                                     \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_addw_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    return zig_wrap_i##w((int##w##_t)((uint##w##_t)lhs + (uint##w##_t)rhs),    \
                         bits);                                                \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_subw_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    return zig_wrap_u##w(lhs - rhs, bits);                                     \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_subw_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    return zig_wrap_i##w((int##w##_t)((uint##w##_t)lhs - (uint##w##_t)rhs),    \
                         bits);                                                \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_mulw_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    return zig_wrap_u##w((PromotedUnsigned)lhs * rhs, bits);                   \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_mulw_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    return zig_wrap_i##w((int##w##_t)((uint##w##_t)lhs * (uint##w##_t)rhs),    \
                         bits);                                                \
  }
...
zig_int_helpers(8, uint8_t)
    ...
    zig_int_helpers(16, uint16_t)
        ...
        zig_int_helpers(32, uint32_t)
            ...
            zig_int_helpers(64, uint64_t)
                ...
                static inline bool zig_addo_u32(uint32_t *res, uint32_t lhs,
                                                uint32_t rhs, uint8_t bits) {
  ...
  *res = zig_wrap_u32(full_res, bits);
  return overflow || full_res < zig_intMin_u(32, bits) ||
         full_res > zig_intMax_u(32, bits);
#else
...
static inline bool zig_addo_i32(int32_t *res, int32_t lhs, int32_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  int32_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  ...
  *res = zig_wrap_i32(full_res, bits);
  return overflow || full_res < zig_intMin_i(32, bits) ||
         full_res > zig_intMax_i(32, bits);
}

static inline bool zig_addo_u64(uint64_t *res, uint64_t lhs, uint64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  uint64_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  *res = zig_wrap_u64(full_res, bits);
  return overflow || full_res < zig_intMin_u(64, bits) ||
         full_res > zig_intMax_u(64, bits);
#else
...
static inline bool zig_addo_i64(int64_t *res, int64_t lhs, int64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  int64_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  ...
  *res = zig_wrap_i64(full_res, bits);
  return overflow || full_res < zig_intMin_i(64, bits) ||
         full_res > zig_intMax_i(64, bits);
}

static inline bool zig_addo_u8(uint8_t *res, uint8_t lhs, uint8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  uint8_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  *res = zig_wrap_u8(full_res, bits);
  return overflow || full_res < zig_intMin_u(8, bits) ||
         full_res > zig_intMax_u(8, bits);
#else
  uint32_t full_res;
...
static inline bool zig_addo_i8(int8_t *res, int8_t lhs, int8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  int8_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  *res = zig_wrap_i8(full_res, bits);
  return overflow || full_res < zig_intMin_i(8, bits) ||
         full_res > zig_intMax_i(8, bits);
#else
  int32_t full_res;
...
static inline bool zig_addo_u16(uint16_t *res, uint16_t lhs, uint16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  uint16_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  *res = zig_wrap_u16(full_res, bits);
  return overflow || full_res < zig_intMin_u(16, bits) ||
         full_res > zig_intMax_u(16, bits);
#else
  uint32_t full_res;
  bool overflow = zig_addo_u32(&full_res, lhs, rhs, bits);
...
static inline bool zig_addo_i16(int16_t *res, int16_t lhs, int16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(add_overflow) || defined(zig_gcc)
  int16_t full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  *res = zig_wrap_i16(full_res, bits);
  return overflow || full_res < zig_intMin_i(16, bits) ||
         full_res > zig_intMax_i(16, bits);
#else
  int32_t full_res;
  bool overflow = zig_addo_i32(&full_res, lhs, rhs, bits);
...
static inline bool zig_subo_u32(uint32_t *res, uint32_t lhs, uint32_t rhs,
                                uint8_t bits) {
...
static inline bool zig_subo_i32(int32_t *res, int32_t lhs, int32_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  int32_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_u64(uint64_t *res, uint64_t lhs, uint64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  uint64_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_i64(int64_t *res, int64_t lhs, int64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  int64_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_u8(uint8_t *res, uint8_t lhs, uint8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  uint8_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_i8(int8_t *res, int8_t lhs, int8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  int8_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_u16(uint16_t *res, uint16_t lhs, uint16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  uint16_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_subo_i16(int16_t *res, int16_t lhs, int16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(sub_overflow) || defined(zig_gcc)
  int16_t full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_u32(uint32_t *res, uint32_t lhs, uint32_t rhs,
                                uint8_t bits) {
  ...
  *res = zig_mulw_u32(lhs, rhs, bits);
  return rhs != UINT32_C(0) && lhs > zig_intMax_u(32, bits) / rhs;
#endif
}

zig_extern int32_t __mulosi4(int32_t lhs, int32_t rhs, int *overflow);
static inline bool zig_mulo_i32(int32_t *res, int32_t lhs, int32_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  int32_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_u64(uint64_t *res, uint64_t lhs, uint64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  uint64_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
  ...
  *res = zig_mulw_u64(lhs, rhs, bits);
  return rhs != UINT64_C(0) && lhs > zig_intMax_u(64, bits) / rhs;
#endif
}

zig_extern int64_t __mulodi4(int64_t lhs, int64_t rhs, int *overflow);
static inline bool zig_mulo_i64(int64_t *res, int64_t lhs, int64_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  int64_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_u8(uint8_t *res, uint8_t lhs, uint8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  uint8_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_i8(int8_t *res, int8_t lhs, int8_t rhs,
                               uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  int8_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_u16(uint16_t *res, uint16_t lhs, uint16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  uint16_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_i16(int16_t *res, int16_t lhs, int16_t rhs,
                                uint8_t bits) {
#if zig_has_builtin(mul_overflow) || defined(zig_gcc)
  int16_t full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
#define zig_int_builtins(w)                                                    \
  static inline bool zig_shlo_u##w(uint##w##_t *res, uint##w##_t lhs,          \
                                   uint8_t rhs, uint8_t bits) {                \
    *res = zig_shlw_u##w(lhs, rhs, bits);                                      \
    return lhs > zig_intMax_u(w, bits) >> rhs;                                 \
  }                                                                            \
                                                                               \
  static inline bool zig_shlo_i##w(int##w##_t *res, int##w##_t lhs,            \
                                   uint8_t rhs, uint8_t bits) {                \
    *res = zig_shlw_i##w(lhs, rhs, bits);                                      \
    ...
    return (lhs & mask) != INT##w##_C(0) && (lhs & mask) != mask;              \
                                                                               ...
                                                                               \
  static inline uint##w##_t zig_shls_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    uint##w##_t res;                                                           \
    if (rhs < bits && !zig_shlo_u##w(&res, lhs, rhs, bits))                    \
      return res;                                                              \
    return lhs == INT##w##_C(0) ? INT##w##_C(0) : zig_intMax_u(w, bits);       \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_shls_i##w(int##w##_t lhs, uint##w##_t rhs,      \
                                         uint8_t bits) {                       \
    int##w##_t res;                                                            \
    if (rhs < bits && !zig_shlo_i##w(&res, lhs, rhs, bits))                    \
      return res;                                                              \
    return lhs == INT##w##_C(0)  ? INT##w##_C(0)                               \
           : lhs < INT##w##_C(0) ? zig_intMin_i(w, bits)                       \
                                 : zig_intMax_i(w, bits);                      \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_adds_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    uint##w##_t res;                                                           \
    return zig_addo_u##w(&res, lhs, rhs, bits) ? zig_intMax_u(w, bits) : res;  \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_adds_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    int##w##_t res;                                                            \
    if (!zig_addo_i##w(&res, lhs, rhs, bits))                                  \
      return res;                                                              \
    return res >= INT##w##_C(0) ? zig_intMin_i(w, bits)                        \
                                : zig_intMax_i(w, bits);                       \
  }                                                                            \
                                                                               \
  static inline uint##w##_t zig_subs_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    uint##w##_t res;                                                           \
    return zig_subo_u##w(&res, lhs, rhs, bits) ? zig_intMin_u(w, bits) : res;  \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_subs_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    int##w##_t res;                                                            \
    ...
    return res >= INT##w##_C(0) ? zig_intMin_i(w, bits)                        \
                                : zig_intMax_i(w, bits);                       \
                                                                               ...
                                                                               \
  static inline uint##w##_t zig_muls_u##w(uint##w##_t lhs, uint##w##_t rhs,    \
                                          uint8_t bits) {                      \
    uint##w##_t res;                                                           \
    return zig_mulo_u##w(&res, lhs, rhs, bits) ? zig_intMax_u(w, bits) : res;  \
  }                                                                            \
                                                                               \
  static inline int##w##_t zig_muls_i##w(int##w##_t lhs, int##w##_t rhs,       \
                                         uint8_t bits) {                       \
    int##w##_t res;                                                            \
    if (!zig_mulo_i##w(&res, lhs, rhs, bits))                                  \
      return res;                                                              \
    return (lhs ^ rhs) < INT##w##_C(0) ? zig_intMin_i(w, bits)                 \
                                       : zig_intMax_i(w, bits);                \
  }
zig_int_builtins(8) zig_int_builtins(16) zig_int_builtins(32)
    zig_int_builtins(64)
...
#else
  static uint8_t const lut[0x10] = {0x0, 0x8, 0x4, 0xc, 0x2, 0xa, 0x6, 0xe,
...
#define zig_builtin_popcount(w)                                                \
  ...
  zig_builtin_popcount_common(w)
#endif
zig_builtin_popcount(8) zig_builtin_popcount(16) zig_builtin_popcount(32)
...
#define zig_builtin_ctz(w)                                                     \
  static inline uint8_t zig_ctz_u##w(uint##w##_t val, uint8_t bits) {          \
    return zig_popcount_u##w(                                                  \
        zig_not_u##w(val, bits) & zig_subw_u##w(val, 1, bits), bits);          \
  }                                                                            \
                                                                               \
  zig_builtin_ctz_common(w)
#endif
        zig_builtin_ctz(8) zig_builtin_ctz(16) zig_builtin_ctz(32)
            zig_builtin_ctz(64)
...
#define zig_builtin_clz(w)                                                     \
  ...
  zig_builtin_clz_common(w)
#endif
                zig_builtin_clz(8) zig_builtin_clz(16) zig_builtin_clz(32)
                    zig_builtin_clz(64)
...
#define zig_bit_int128(Type, operation, operator)                              \
  static inline zig_##Type zig_##operation##_##Type(zig_##Type lhs,            \
                                                    zig_##Type rhs) {          \
    return lhs operator rhs;                                                   \
  }
...
#if defined(zig_msvc) /* MSVC doesn't allow struct literals in constant        \
                         expressions */
...
#define zig_cmp_int128(Type)                                                   \
  static inline int32_t zig_cmp_##Type(zig_##Type lhs, zig_##Type rhs) {       \
    return (lhs.hi == rhs.hi) ? (lhs.lo > rhs.lo) - (lhs.lo < rhs.lo)          \
                              : (lhs.hi > rhs.hi) - (lhs.hi < rhs.hi);         \
  }
#define zig_bit_int128(Type, operation, operator)                              \
  static inline zig_##Type zig_##operation##_##Type(zig_##Type lhs,            \
                                                    zig_##Type rhs) {          \
    return (zig_##Type){.hi = lhs.hi operator rhs.hi,                          \
                        .lo = lhs.lo operator rhs.lo };                        \
  }

#endif /* zig_has_int128 */

#define zig_intMin_u128 zig_make_u128(zig_intMin_u64, zig_intMin_u64)
#define zig_intMax_u128 zig_make_u128(zig_intMax_u64, zig_intMax_u64)
#define zig_intMin_i128 zig_make_i128(zig_intMin_i64, zig_intMin_u64)
#define zig_intMax_i128 zig_make_i128(zig_intMax_i64, zig_intMax_u64)

zig_cmp_int128(u128) zig_cmp_int128(i128)

    zig_bit_int128(u128, and, &) zig_bit_int128(i128, and, &)

        zig_bit_int128(u128, or, |) zig_bit_int128(i128, or, |)

            zig_bit_int128(u128, xor, ^) zig_bit_int128(i128, xor, ^)
...
static inline zig_u128 zig_not_u128(zig_u128 val, uint8_t bits) {
  return val ^ zig_intMax_u(128, bits);
}
...
#else
  zig_i128 sign_mask =
      lhs < zig_make_i128(0, 0) ? -zig_make_i128(0, 1) : zig_make_i128(0, 0);
  return ((lhs ^ sign_mask) >> rhs) ^ sign_mask;
#endif
}
...
static inline zig_u128 zig_not_u128(zig_u128 val, uint8_t bits) {
  return (zig_u128){.hi = zig_not_u64(val.hi, bits - UINT8_C(64)),
                    .lo = zig_not_u64(val.lo, UINT8_C(64))};
}

static inline zig_i128 zig_not_i128(zig_i128 val, uint8_t bits) {
  return (zig_i128){.hi = zig_not_i64(val.hi, bits - UINT8_C(64)),
                    .lo = zig_not_u64(val.lo, UINT8_C(64))};
}
...
static inline zig_u128 zig_shr_u128(zig_u128 lhs, uint8_t rhs) {
  if (rhs == UINT8_C(0))
    return lhs;
  if (rhs >= UINT8_C(64))
    return (zig_u128){.hi = zig_intMin_u64,
                      .lo = lhs.hi >> (rhs - UINT8_C(64))};
  return (zig_u128){.hi = lhs.hi >> rhs,
                    .lo = lhs.hi << (UINT8_C(64) - rhs) | lhs.lo >> rhs};
}
...
static inline zig_u128 zig_shl_u128(zig_u128 lhs, uint8_t rhs) {
  if (rhs == UINT8_C(0))
    return lhs;
  if (rhs >= UINT8_C(64))
    return (zig_u128){.hi = lhs.lo << (rhs - UINT8_C(64)),
                      .lo = zig_intMin_u64};
  return (zig_u128){.hi = lhs.hi << rhs | lhs.lo >> (UINT8_C(64) - rhs),
                    .lo = lhs.lo << rhs};
}
...
static inline zig_i128 zig_shr_i128(zig_i128 lhs, uint8_t rhs) {
  if (rhs == UINT8_C(0))
    return lhs;
  if (rhs >= UINT8_C(64))
    return (zig_i128){.hi = zig_shr_i64(lhs.hi, 63),
                      .lo = zig_shr_i64(lhs.hi, (rhs - UINT8_C(64)))};
  return (zig_i128){.hi = zig_shr_i64(lhs.hi, rhs),
                    .lo = lhs.lo >> rhs | (uint64_t)lhs.hi
                                              << (UINT8_C(64) - rhs)};
}
...
static inline zig_i128 zig_shl_i128(zig_i128 lhs, uint8_t rhs) {
  if (rhs == UINT8_C(0))
    return lhs;
  if (rhs >= UINT8_C(64))
    return (zig_i128){.hi = lhs.lo << (rhs - UINT8_C(64)),
                      .lo = zig_intMin_u64};
  return (zig_i128){.hi = lhs.hi << rhs | lhs.lo >> (UINT8_C(64) - rhs),
                    .lo = lhs.lo << rhs};
}
...
static zig_u128 zig_mul_u128(zig_u128 lhs, zig_u128 rhs) {
  return zig_bitCast_u128(
      zig_mul_i128(zig_bitCast_i128(lhs), zig_bitCast_i128(rhs)));
}
...
static inline zig_i128 zig_div_floor_i128(zig_i128 lhs, zig_i128 rhs) {
  ...
  int64_t mask =
      zig_or_u64((uint64_t)zig_hi_i128(rem), zig_lo_i128(rem)) != UINT64_C(0)
          ? zig_shr_i64(zig_xor_i64(zig_hi_i128(lhs), zig_hi_i128(rhs)),
                        UINT8_C(63))
          : INT64_C(0);
  return zig_add_i128(zig_div_trunc_i128(lhs, rhs),
                      zig_make_i128(mask, (uint64_t)mask));
}
  ...
  int64_t mask =
      zig_or_u64((uint64_t)zig_hi_i128(rem), zig_lo_i128(rem)) != UINT64_C(0)
          ? zig_shr_i64(zig_xor_i64(zig_hi_i128(lhs), zig_hi_i128(rhs)),
                        UINT8_C(63))
          : INT64_C(0);
  return zig_add_i128(rem,
                      zig_and_i128(rhs, zig_make_i128(mask, (uint64_t)mask)));
}
...
static inline zig_u128 zig_wrap_u128(zig_u128 val, uint8_t bits) {
  return zig_and_u128(val, zig_intMax_u(128, bits));
}

static inline zig_i128 zig_wrap_i128(zig_i128 val, uint8_t bits) {
  if (bits > UINT8_C(64))
    return zig_make_i128(zig_wrap_i64(zig_hi_i128(val), bits - UINT8_C(64)),
                         zig_lo_i128(val));
...
static inline zig_i128 zig_shlw_i128(zig_i128 lhs, uint8_t rhs, uint8_t bits) {
  return zig_wrap_i128(
      zig_bitCast_i128(zig_shl_u128(zig_bitCast_u128(lhs), rhs)), bits);
}
...
static inline zig_i128 zig_addw_i128(zig_i128 lhs, zig_i128 rhs, uint8_t bits) {
  return zig_wrap_i128(zig_bitCast_i128(zig_add_u128(zig_bitCast_u128(lhs),
                                                     zig_bitCast_u128(rhs))),
                       bits);
}
...
static inline zig_i128 zig_subw_i128(zig_i128 lhs, zig_i128 rhs, uint8_t bits) {
  return zig_wrap_i128(zig_bitCast_i128(zig_sub_u128(zig_bitCast_u128(lhs),
                                                     zig_bitCast_u128(rhs))),
                       bits);
}
...
static inline zig_i128 zig_mulw_i128(zig_i128 lhs, zig_i128 rhs, uint8_t bits) {
  return zig_wrap_i128(zig_bitCast_i128(zig_mul_u128(zig_bitCast_u128(lhs),
                                                     zig_bitCast_u128(rhs))),
                       bits);
}
...
static inline bool zig_addo_u128(zig_u128 *res, zig_u128 lhs, zig_u128 rhs,
  ...
  *res = zig_wrap_u128(full_res, bits);
  return overflow || full_res < zig_intMin_u(128, bits) ||
         full_res > zig_intMax_u(128, bits);
#else
...
static inline bool zig_addo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
                                 uint8_t bits) {
#if zig_has_builtin(add_overflow)
  zig_i128 full_res;
  bool overflow = __builtin_add_overflow(lhs, rhs, &full_res);
  ...
  *res = zig_wrap_i128(full_res, bits);
  return overflow || full_res < zig_intMin_i(128, bits) ||
         full_res > zig_intMax_i(128, bits);
}
...
static inline bool zig_subo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
                                 uint8_t bits) {
#if zig_has_builtin(sub_overflow)
  zig_i128 full_res;
  bool overflow = __builtin_sub_overflow(lhs, rhs, &full_res);
...
static inline bool zig_mulo_u128(zig_u128 *res, zig_u128 lhs, zig_u128 rhs,
                                 uint8_t bits) {
  ...
  *res = zig_mulw_u128(lhs, rhs, bits);
  return rhs != zig_make_u128(0, 0) && lhs > zig_intMax_u(128, bits) / rhs;
#endif
}

zig_extern zig_i128 __muloti4(zig_i128 lhs, zig_i128 rhs, int *overflow);
static inline bool zig_mulo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
                                 uint8_t bits) {
#if zig_has_builtin(mul_overflow)
  zig_i128 full_res;
  bool overflow = __builtin_mul_overflow(lhs, rhs, &full_res);
...
static inline bool zig_addo_u128(zig_u128 *res, zig_u128 lhs, zig_u128 rhs,
  ...
  return overflow ^ zig_addo_u64(&res->hi, hi,
                                 zig_addo_u64(&res->lo, lhs.lo, rhs.lo, 64),
                                 bits - 64);
}
...
static inline bool zig_addo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
  ...
  return overflow ^ zig_addo_i64(&res->hi, hi,
                                 zig_addo_u64(&res->lo, lhs.lo, rhs.lo, 64),
                                 bits - 64);
}
...
static inline bool zig_subo_u128(zig_u128 *res, zig_u128 lhs, zig_u128 rhs,
  ...
  return overflow ^ zig_subo_u64(&res->hi, hi,
                                 zig_subo_u64(&res->lo, lhs.lo, rhs.lo, 64),
                                 bits - 64);
}
...
static inline bool zig_subo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
  ...
  return overflow ^ zig_subo_i64(&res->hi, hi,
                                 zig_subo_u64(&res->lo, lhs.lo, rhs.lo, 64),
                                 bits - 64);
}
...
static inline bool zig_mulo_u128(zig_u128 *res, zig_u128 lhs, zig_u128 rhs,
  ...
  *res = zig_mulw_u128(lhs, rhs, bits);
  return zig_cmp_u128(*res, zig_make_u128(0, 0)) != INT32_C(0) &&
         zig_cmp_u128(lhs, zig_div_trunc_u128(zig_intMax_u(128, bits), rhs)) >
             INT32_C(0);
}
...
static inline bool zig_mulo_i128(zig_i128 *res, zig_i128 lhs, zig_i128 rhs,
  ...
  bool overflow =
      overflow_int != 0 ||
      zig_cmp_i128(full_res, zig_intMin_i(128, bits)) < INT32_C(0) ||
      zig_cmp_i128(full_res, zig_intMax_i(128, bits)) > INT32_C(0);
  *res = zig_wrap_i128(full_res, bits);
  return overflow;
}
...
static inline bool zig_shlo_u128(zig_u128 *res, zig_u128 lhs, uint8_t rhs,
                                 uint8_t bits) {
  *res = zig_shlw_u128(lhs, rhs, bits);
  return zig_cmp_u128(lhs, zig_shr_u128(zig_intMax_u(128, bits), rhs)) >
         INT32_C(0);
}

static inline bool zig_shlo_i128(zig_i128 *res, zig_i128 lhs, uint8_t rhs,
                                 uint8_t bits) {
  *res = zig_shlw_i128(lhs, rhs, bits);
  zig_i128 mask =
      zig_bitCast_i128(zig_shl_u128(zig_intMax_u128, bits - rhs - UINT8_C(1)));
  return zig_cmp_i128(zig_and_i128(lhs, mask), zig_make_i128(0, 0)) !=
             INT32_C(0) &&
...
static inline zig_u128 zig_shls_u128(zig_u128 lhs, zig_u128 rhs, uint8_t bits) {
  ...
  if (zig_cmp_u128(rhs, zig_make_u128(0, bits)) < INT32_C(0) &&
      !zig_shlo_u128(&res, lhs, (uint8_t)zig_lo_u128(rhs), bits))
    return res;
  switch (zig_cmp_u128(lhs, zig_make_u128(0, 0))) {
  case 0:
    return zig_make_u128(0, 0);
  case 1:
    return zig_intMax_u(128, bits);
  default:
    zig_unreachable();
  }
}
...
static inline zig_i128 zig_shls_i128(zig_i128 lhs, zig_u128 rhs, uint8_t bits) {
  zig_i128 res;
  if (zig_cmp_u128(rhs, zig_make_u128(0, bits)) < INT32_C(0) &&
      !zig_shlo_i128(&res, lhs, (uint8_t)zig_lo_u128(rhs), bits))
    return res;
  switch (zig_cmp_i128(lhs, zig_make_i128(0, 0))) {
  case -1:
    return zig_intMin_i(128, bits);
  case 0:
    return zig_make_i128(0, 0);
  case 1:
    return zig_intMax_i(128, bits);
  default:
    zig_unreachable();
  }
}
...
static inline zig_u128 zig_adds_u128(zig_u128 lhs, zig_u128 rhs, uint8_t bits) {
  zig_u128 res;
  return zig_addo_u128(&res, lhs, rhs, bits) ? zig_intMax_u(128, bits) : res;
}
...
static inline zig_i128 zig_adds_i128(zig_i128 lhs, zig_i128 rhs, uint8_t bits) {
  ...
  if (!zig_addo_i128(&res, lhs, rhs, bits))
    return res;
  return zig_cmp_i128(res, zig_make_i128(0, 0)) >= INT32_C(0)
             ? zig_intMin_i(128, bits)
             : zig_intMax_i(128, bits);
}

static inline zig_u128 zig_subs_u128(zig_u128 lhs, zig_u128 rhs, uint8_t bits) {
  zig_u128 res;
  return zig_subo_u128(&res, lhs, rhs, bits) ? zig_intMin_u(128, bits) : res;
}
...
static inline zig_u128 zig_muls_u128(zig_u128 lhs, zig_u128 rhs, uint8_t bits) {
  zig_u128 res;
  return zig_mulo_u128(&res, lhs, rhs, bits) ? zig_intMax_u(128, bits) : res;
}
...
static inline zig_i128 zig_muls_i128(zig_i128 lhs, zig_i128 rhs, uint8_t bits) {
  ...
  if (!zig_mulo_i128(&res, lhs, rhs, bits))
    return res;
  return zig_cmp_i128(zig_xor_i128(lhs, rhs), zig_make_i128(0, 0)) < INT32_C(0)
             ? zig_intMin_i(128, bits)
             : zig_intMax_i(128, bits);
}
...
static inline zig_u128 zig_bit_reverse_u128(zig_u128 val, uint8_t bits) {
  return zig_shr_u128(
      zig_make_u128(zig_bit_reverse_u64(zig_lo_u128(val), UINT8_C(64)),
...
static inline int32_t zig_cmp_big(const void *lhs, const void *rhs,
                                  bool is_signed, uint16_t bits) {
...
static inline void zig_and_big(void *res, const void *lhs, const void *rhs,
                               bool is_signed, uint16_t bits) {
...
static inline void zig_or_big(void *res, const void *lhs, const void *rhs,
                              bool is_signed, uint16_t bits) {
  uint8_t *res_bytes = res;
  const uint8_t *lhs_bytes = lhs;
  const uint8_t *rhs_bytes = rhs;
...
static inline void zig_xor_big(void *res, const void *lhs, const void *rhs,
                               bool is_signed, uint16_t bits) {
  uint8_t *res_bytes = res;
  const uint8_t *lhs_bytes = lhs;
  const uint8_t *rhs_bytes = rhs;
...
static inline bool zig_addo_big(void *res, const void *lhs, const void *rhs,
                                bool is_signed, uint16_t bits) {
  uint8_t *res_bytes = res;
  const uint8_t *lhs_bytes = lhs;
  const uint8_t *rhs_bytes = rhs;
  ...
  while (remaining_bytes >= 128 / CHAR_BIT) {
    uint8_t limb_bits =
        128 - (remaining_bytes == 128 / CHAR_BIT ? top_bits : 0);
    ...
    if (remaining_bytes == 128 / CHAR_BIT && is_signed) {
      ...
      overflow =
          ...
          zig_addo_i128(
              &res_limb, tmp_limb,
              zig_make_i128(INT64_C(0), overflow ? UINT64_C(1) : UINT64_C(0)),
              limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      zig_u128 res_limb;
      ...
      overflow =
          ...
          zig_addo_u128(
              &res_limb, tmp_limb,
              zig_make_u128(UINT64_C(0), overflow ? UINT64_C(1) : UINT64_C(0)),
              limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 64 / CHAR_BIT) {
    ...
    if (remaining_bytes == 64 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_addo_i64(&res_limb, tmp_limb,
                              overflow ? INT64_C(1) : INT64_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint64_t res_limb;
      uint64_t tmp_limb;
      ...
      overflow = limb_overflow ^
                 zig_addo_u64(&res_limb, tmp_limb,
                              overflow ? UINT64_C(1) : UINT64_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 32 / CHAR_BIT) {
    ...
    if (remaining_bytes == 32 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_addo_i32(&res_limb, tmp_limb,
                              overflow ? INT32_C(1) : INT32_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint32_t res_limb;
      uint32_t tmp_limb;
      ...
      overflow = limb_overflow ^
                 zig_addo_u32(&res_limb, tmp_limb,
                              overflow ? UINT32_C(1) : UINT32_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 16 / CHAR_BIT) {
    ...
    if (remaining_bytes == 16 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_addo_i16(&res_limb, tmp_limb,
                              overflow ? INT16_C(1) : INT16_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint16_t res_limb;
      uint16_t tmp_limb;
      ...
      overflow = limb_overflow ^
                 zig_addo_u16(&res_limb, tmp_limb,
                              overflow ? UINT16_C(1) : UINT16_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 8 / CHAR_BIT) {
    ...
    if (remaining_bytes == 8 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^ zig_addo_i8(&res_limb, tmp_limb,
                                             overflow ? INT8_C(1) : INT8_C(0),
                                             limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint8_t res_limb;
      ...
      overflow = limb_overflow ^ zig_addo_u8(&res_limb, tmp_limb,
                                             overflow ? UINT8_C(1) : UINT8_C(0),
                                             limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
...
static inline bool zig_subo_big(void *res, const void *lhs, const void *rhs,
                                bool is_signed, uint16_t bits) {
  uint8_t *res_bytes = res;
  const uint8_t *lhs_bytes = lhs;
  const uint8_t *rhs_bytes = rhs;
  ...
  while (remaining_bytes >= 64 / CHAR_BIT) {
    ...
    if (remaining_bytes == 64 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_subo_i64(&res_limb, tmp_limb,
                              overflow ? INT64_C(1) : INT64_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint64_t res_limb;
      ...
      overflow = limb_overflow ^
                 zig_subo_u64(&res_limb, tmp_limb,
                              overflow ? UINT64_C(1) : UINT64_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 32 / CHAR_BIT) {
    ...
    if (remaining_bytes == 32 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_subo_i32(&res_limb, tmp_limb,
                              overflow ? INT32_C(1) : INT32_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint32_t res_limb;
      ...
      overflow = limb_overflow ^
                 zig_subo_u32(&res_limb, tmp_limb,
                              overflow ? UINT32_C(1) : UINT32_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 16 / CHAR_BIT) {
    ...
    if (remaining_bytes == 16 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^
                 zig_subo_i16(&res_limb, tmp_limb,
                              overflow ? INT16_C(1) : INT16_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      uint16_t res_limb;
      ...
      overflow = limb_overflow ^
                 zig_subo_u16(&res_limb, tmp_limb,
                              overflow ? UINT16_C(1) : UINT16_C(0), limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
  ...
  while (remaining_bytes >= 8 / CHAR_BIT) {
    ...
    if (remaining_bytes == 8 / CHAR_BIT && is_signed) {
      ...
      overflow = limb_overflow ^ zig_subo_i8(&res_limb, tmp_limb,
                                             overflow ? INT8_C(1) : INT8_C(0),
                                             limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    } else {
      ...
      overflow = limb_overflow ^ zig_subo_u8(&res_limb, tmp_limb,
                                             overflow ? UINT8_C(1) : UINT8_C(0),
                                             limb_bits);
      memcpy(&res_bytes[byte_offset], &res_limb, sizeof(res_limb));
    }
...
static inline void zig_addw_big(void *res, const void *lhs, const void *rhs,
                                bool is_signed, uint16_t bits) {
  (void)zig_addo_big(res, lhs, rhs, is_signed, bits);
}

static inline void zig_subw_big(void *res, const void *lhs, const void *rhs,
                                bool is_signed, uint16_t bits) {
  (void)zig_subo_big(res, lhs, rhs, is_signed, bits);
}

zig_extern void __udivei4(uint32_t *res, const uint32_t *lhs,
                          const uint32_t *rhs, uintptr_t bits);
static inline void zig_div_trunc_big(void *res, const void *lhs,
                                     const void *rhs, bool is_signed,
                                     uint16_t bits) {
  if (!is_signed) {
...
static inline void zig_div_floor_big(void *res, const void *lhs,
                                     const void *rhs, bool is_signed,
                                     uint16_t bits) {
  if (!is_signed) {
...
zig_extern void __umodei4(uint32_t *res, const uint32_t *lhs,
                          const uint32_t *rhs, uintptr_t bits);
static inline void zig_rem_big(void *res, const void *lhs, const void *rhs,
                               bool is_signed, uint16_t bits) {
  if (!is_signed) {
...
static inline void zig_mod_big(void *res, const void *lhs, const void *rhs,
                               bool is_signed, uint16_t bits) {
  if (!is_signed) {
...
static inline uint16_t zig_clz_big(const void *val, bool is_signed,
                                   uint16_t bits) {
...
static inline uint16_t zig_ctz_big(const void *val, bool is_signed,
                                   uint16_t bits) {
  const uint8_t *val_bytes = val;
  uint16_t byte_offset = 0;
  uint16_t remaining_bytes = zig_int_bytes(bits);
...
static inline uint16_t zig_popcount_big(const void *val, bool is_signed,
                                        uint16_t bits) {
  const uint8_t *val_bytes = val;
  uint16_t byte_offset = 0;
  uint16_t remaining_bytes = zig_int_bytes(bits);
...
#define __builtin_infl() zig_msvc_flt_infl
    ...
    defined(zig_gcc)
#define zig_make_special_f16(sign, name, arg, repr)                            \
  sign zig_make_f16(__builtin_##name, )(arg)
#define zig_make_special_f32(sign, name, arg, repr)                            \
  sign zig_make_f32(__builtin_##name, )(arg)
#define zig_make_special_f64(sign, name, arg, repr)                            \
  sign zig_make_f64(__builtin_##name, )(arg)
#define zig_make_special_f80(sign, name, arg, repr)                            \
  sign zig_make_f80(__builtin_##name, )(arg)
#define zig_make_special_f128(sign, name, arg, repr)                           \
  sign zig_make_f128(__builtin_##name, )(arg)
#else
...
#define zig_init_special_f16(sign, name, arg, repr)                            \
  zig_make_special_f16(sign, name, arg, repr)
#if FLT_MANT_DIG == 11
...
#if defined(zig_msvc)
#define zig_init_special_f32(sign, name, arg, repr)                            \
  sign zig_make_f32(zig_msvc_flt_##name, )
#else
#define zig_init_special_f32(sign, name, arg, repr)                            \
  zig_make_special_f32(sign, name, arg, repr)
#endif
#if FLT_MANT_DIG == 24
...
#if defined(zig_msvc)
#define zig_init_special_f64(sign, name, arg, repr)                            \
  sign zig_make_f64(zig_msvc_flt_##name, )
#else
#define zig_init_special_f64(sign, name, arg, repr)                            \
  zig_make_special_f64(sign, name, arg, repr)
#endif
#if FLT_MANT_DIG == 53
...
#define zig_init_special_f80(sign, name, arg, repr)                            \
  zig_make_special_f80(sign, name, arg, repr)
#if FLT_MANT_DIG == 64
...
#define zig_init_special_f128(sign, name, arg, repr)                           \
  zig_make_special_f128(sign, name, arg, repr)
...
typedef
    __attribute__((__vector_size__(2 * sizeof(uint64_t)))) uint64_t zig_v2u64;
...
zig_bitCast_float(f16, uint16_t) zig_bitCast_float(f32, uint32_t)
    zig_bitCast_float(f64, uint64_t) zig_bitCast_float(
        f80, zig_u128) zig_bitCast_float(f128, zig_u128)

#define zig_convert_builtin(ExternResType, ResType, operation, ExternArgType,  \
                            ArgType, version)                                  \
  zig_extern ExternResType zig_expand_concat(                                  \
      zig_expand_concat(                                                       \
          zig_expand_concat(__##operation, zig_compiler_rt_abbrev_##ArgType),  \
          zig_compiler_rt_abbrev_##ResType),                                   \
      version)(ExternArgType);                                                 \
  static inline ResType zig_expand_concat(                                     \
      zig_expand_concat(zig_##operation, zig_compiler_rt_abbrev_##ArgType),    \
    ...
    extern_res = zig_expand_concat(                                            \
        zig_expand_concat(zig_expand_concat(__##operation,                     \
    ...
    return extern_res;                                                         \
        ...
        zig_convert_builtin(
            zig_compiler_rt_f16, zig_f16, trunc, zig_f32, zig_f32,
            2) zig_convert_builtin(zig_compiler_rt_f16, zig_f16, trunc, zig_f64,
                                   zig_f64,
                                   2) zig_convert_builtin(zig_f16, zig_f16,
                                                          trunc, zig_f80,
                                                          zig_f80, 2)
            zig_convert_builtin(zig_f16, zig_f16, trunc, zig_f128, zig_f128,
                                2) zig_convert_builtin(zig_f32, zig_f32, extend,
                                                       zig_compiler_rt_f16,
                                                       zig_f16, 2)
                zig_convert_builtin(zig_f32, zig_f32, trunc, zig_f80, zig_f80,
                                    2) zig_convert_builtin(zig_f32, zig_f32,
                                                           trunc, zig_f128,
                                                           zig_f128, 2)
                    zig_convert_builtin(zig_f64, zig_f64, extend,
                                        zig_compiler_rt_f16, zig_f16,
                                        2) zig_convert_builtin(zig_f64, zig_f64,
                                                               trunc, zig_f80,
                                                               zig_f80, 2)
                        zig_convert_builtin(
                            zig_f64, zig_f64, trunc, zig_f128, zig_f128,
                            2) zig_convert_builtin(zig_f80, zig_f80, extend,
                                                   zig_f16, zig_f16, 2)
                            zig_convert_builtin(
                                zig_f80, zig_f80, extend, zig_f32, zig_f32,
                                2) zig_convert_builtin(zig_f80, zig_f80, extend,
                                                       zig_f64, zig_f64, 2)
                                zig_convert_builtin(zig_f80, zig_f80, trunc,
                                                    zig_f128, zig_f128, 2)
                                    zig_convert_builtin(zig_f128, zig_f128,
                                                        extend, zig_f16,
                                                        zig_f16, 2)
                                        zig_convert_builtin(zig_f128, zig_f128,
                                                            extend, zig_f32,
                                                            zig_f32, 2)
                                            zig_convert_builtin(zig_f128,
                                                                zig_f128,
                                                                extend, zig_f64,
                                                                zig_f64, 2)
                                                zig_convert_builtin(zig_f128,
...
static inline zig_f64 zig_extendsfdf(zig_f32 arg) { return __aeabi_f2d(arg); }
                                                    ...
                                                    zig_convert_builtin(
                                                        zig_f32, zig_f32, trunc,
                                                        zig_f64, zig_f64,
                                                        2) zig_convert_builtin(zig_f64,
                                                                               zig_f64,
                                                                               extend,
                                                                               zig_f32,
                                                                               zig_f32,
...
#define zig_float_negate_builtin(w, c, sb)                                     \
  static inline zig_f##w zig_neg_f##w(zig_f##w arg) {                          \
    return zig_expand_concat(zig_float_negate_builtin_, zig_has_f##w)(w, c,    \
                                                                      sb);     \
  }
zig_float_negate_builtin(16, , UINT16_C(1) << 15)
    zig_float_negate_builtin(32, , UINT32_C(1) << 31)
        zig_float_negate_builtin(64, , UINT64_C(1) << 63)
            zig_float_negate_builtin(80, zig_make_u128,
                                     (UINT64_C(1) << 15, UINT64_C(0)))
                zig_float_negate_builtin(128, zig_make_u128,
                                         (UINT64_C(1) << 63, UINT64_C(0)))

#define zig_float_less_builtin_0(Type, operation)                              \
  zig_extern int32_t zig_expand_concat(                                        \
      zig_expand_concat(__##operation, zig_compiler_rt_abbrev_zig_##Type),     \
      2)(zig_##Type, zig_##Type);                                              \
  static inline int32_t zig_##operation##_##Type(zig_##Type lhs,               \
                                                 zig_##Type rhs) {             \
    return zig_expand_concat(                                                  \
        zig_expand_concat(__##operation, zig_compiler_rt_abbrev_zig_##Type),   \
        2)(lhs, rhs);                                                          \
  }
...
#define zig_float_binary_builtin_0(Type, operation, operator)                  \
  zig_extern zig_##Type zig_expand_concat(                                     \
      zig_expand_concat(__##operation, zig_compiler_rt_abbrev_zig_##Type),     \
...
#define zig_common_float_builtins(w)                                           \
  zig_convert_builtin(int64_t, int64_t, fix, zig_f##w,                         \
                      zig_f##w, ) zig_convert_builtin(zig_i128, zig_i128, fix, \
                                                      zig_f##w, zig_f##w, )    \
      zig_convert_builtin(zig_u128, zig_u128, fixuns, zig_f##w,                \
                          zig_f##w, ) zig_convert_builtin(zig_f##w, zig_f##w,  \
                                                          float, int64_t,      \
                                                          int64_t, )           \
          zig_convert_builtin(zig_f##w, zig_f##w, float, zig_i128, zig_i128, ) \
              zig_convert_builtin(                                             \
                  zig_f##w, zig_f##w, floatun, zig_u128,                       \
                  zig_u128, ) zig_expand_concat(zig_float_less_builtin_,       \
                                                zig_has_f##w)(                 \
                  f##w, cmp) zig_expand_concat(zig_float_less_builtin_,        \
                                               zig_has_f##w)(                  \
                  f##w, ne) zig_expand_concat(zig_float_less_builtin_,         \
                                              zig_has_f##w)(                   \
                  f##w, eq) zig_expand_concat(zig_float_less_builtin_,         \
                                              zig_has_f##w)(                   \
                  f##w, lt) zig_expand_concat(zig_float_less_builtin_,         \
                                              zig_has_f##w)(                   \
                  f##w, le) zig_expand_concat(zig_float_greater_builtin_,      \
                                              zig_has_f##w)(                   \
                  f##w, gt) zig_expand_concat(zig_float_greater_builtin_,      \
                                              zig_has_f##w)(                   \
                  f##w, ge) zig_expand_concat(zig_float_binary_builtin_,       \
                                              zig_has_f##w)(                   \
                  f##w, add, +) zig_expand_concat(zig_float_binary_builtin_,   \
                                                  zig_has_f##w)(               \
                  f##w, sub, -) zig_expand_concat(zig_float_binary_builtin_,   \
                                                  zig_has_f##w)(               \
                  f##w, mul, *) zig_expand_concat(zig_float_binary_builtin_,   \
                                                  zig_has_f##w)(f##w, div, /)  \
                  zig_expand_concat(                                           \
                      zig_expand_import_,                                      \
                      zig_expand_has_builtin(zig_libc_name_f##w(sqrt)))(       \
                      zig_f##w, zig_sqrt_f##w, zig_libc_name_f##w(sqrt),       \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(sin)))(    \
                      zig_f##w, zig_sin_f##w, zig_libc_name_f##w(sin),         \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(cos)))(    \
                      zig_f##w, zig_cos_f##w, zig_libc_name_f##w(cos),         \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(tan)))(    \
                      zig_f##w, zig_tan_f##w, zig_libc_name_f##w(tan),         \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(exp)))(    \
                      zig_f##w, zig_exp_f##w, zig_libc_name_f##w(exp),         \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(exp2)))(   \
                      zig_f##w, zig_exp2_f##w, zig_libc_name_f##w(exp2),       \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(log)))(    \
                      zig_f##w, zig_log_f##w, zig_libc_name_f##w(log),         \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(log2)))(   \
                      zig_f##w, zig_log2_f##w, zig_libc_name_f##w(log2),       \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(log10)))(  \
                      zig_f##w, zig_log10_f##w, zig_libc_name_f##w(log10),     \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(fabs)))(   \
                      zig_f##w, zig_abs_f##w, zig_libc_name_f##w(fabs),        \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(floor)))(  \
                      zig_f##w, zig_floor_f##w, zig_libc_name_f##w(floor),     \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(ceil)))(   \
                      zig_f##w, zig_ceil_f##w, zig_libc_name_f##w(ceil),       \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(round)))(  \
                      zig_f##w, zig_round_f##w, zig_libc_name_f##w(round),     \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(trunc)))(  \
                      zig_f##w, zig_trunc_f##w, zig_libc_name_f##w(trunc),     \
                      (zig_f##w x),                                            \
                      (x)) zig_expand_concat(zig_expand_import_,               \
                                             zig_expand_has_builtin(           \
                                                 zig_libc_name_f##w(fmod)))(   \
                      zig_f##w, zig_fmod_f##w, zig_libc_name_f##w(fmod),       \
                      (zig_f##w x, zig_f##w y), (x, y))                        \
                      zig_expand_concat(                                       \
                          zig_expand_import_,                                  \
                          zig_expand_has_builtin(zig_libc_name_f##w(fmin)))(   \
                          zig_f##w, zig_min_f##w, zig_libc_name_f##w(fmin),    \
                          (zig_f##w x, zig_f##w y),                            \
                          (x, y)) zig_expand_concat(zig_expand_import_,        \
                                                    zig_expand_has_builtin(    \
                                                        zig_libc_name_f##w(    \
                                                            fmax)))(           \
                          zig_f##w, zig_max_f##w, zig_libc_name_f##w(fmax),    \
                          (zig_f##w x, zig_f##w y), (x, y))                    \
                          zig_expand_concat(zig_expand_import_,                \
                                            zig_expand_has_builtin(            \
                                                zig_libc_name_f##w(fma)))(     \
                              zig_f##w, zig_fma_f##w, zig_libc_name_f##w(fma), \
                              (zig_f##w x, zig_f##w y, zig_f##w z), (x, y, z)) \
                                                                               \
                              static inline zig_f##w                           \
                              zig_div_trunc_f##w(zig_f##w lhs, zig_f##w rhs) { \
  ...
  static inline zig_f##w zig_mod_f##w(zig_f##w lhs, zig_f##w rhs) {            \
    return zig_sub_f##w(lhs, zig_mul_f##w(zig_div_floor_f##w(lhs, rhs), rhs)); \
                    ...
                    zig_common_float_builtins(16) zig_common_float_builtins(32)
...
#define zig_float_builtins(w)                                                  \
  zig_convert_builtin(int32_t, int32_t, fix, zig_f##w, zig_f##w, )             \
      zig_convert_builtin(uint32_t, uint32_t, fixuns, zig_f##w, zig_f##w, )    \
          zig_convert_builtin(uint64_t, uint64_t, fixuns, zig_f##w,            \
                              zig_f##w, )                                      \
              zig_convert_builtin(zig_f##w, zig_f##w, float, int32_t,          \
                                  int32_t, )                                   \
                  zig_convert_builtin(zig_f##w, zig_f##w, floatun, uint32_t,   \
                                      uint32_t, )                              \
                      zig_convert_builtin(zig_f##w, zig_f##w, floatun,         \
                                          uint64_t, uint64_t, )
                                    zig_float_builtins(16)
                                        zig_float_builtins(80)
                                            zig_float_builtins(128)
                                                ...
                                                zig_extern
    zig_callconv(pcs("aapcs")) int32_t __aeabi_f2iz(zig_f32);
...
static inline uint32_t zig_fixunssfsi(zig_f32 arg) {
  return __aeabi_f2uiz(arg);
}

zig_extern zig_callconv(pcs("aapcs")) uint64_t __aeabi_f2ulz(zig_f32);
static inline uint64_t zig_fixunssfdi(zig_f32 arg) {
  return __aeabi_f2ulz(arg);
}
...
static inline zig_f32 zig_floatunsisf(uint32_t arg) {
  return __aeabi_ui2f(arg);
}

zig_extern zig_callconv(pcs("aapcs")) zig_f32 __aeabi_ul2f(uint64_t);
static inline zig_f32 zig_floatundisf(uint64_t arg) {
  return __aeabi_ul2f(arg);
}
...
static inline uint32_t zig_fixunsdfsi(zig_f64 arg) {
  return __aeabi_d2uiz(arg);
}

zig_extern zig_callconv(pcs("aapcs")) uint64_t __aeabi_d2ulz(zig_f64);
static inline uint64_t zig_fixunsdfdi(zig_f64 arg) {
  return __aeabi_d2ulz(arg);
}
...
static inline zig_f64 zig_floatunsidf(uint32_t arg) {
  return __aeabi_ui2d(arg);
}

zig_extern zig_callconv(pcs("aapcs")) zig_f64 __aeabi_ul2d(uint64_t);
static inline zig_f64 zig_floatundidf(uint64_t arg) {
  return __aeabi_ul2d(arg);
}
...
/* Define fallback implementations first that can later be undef'd on compilers
 * with builtin support. */
/* Note that zig_atomicrmw_expected is needed to handle aliasing between res and
 * arg. */
#define zig_atomicrmw_xchg_float(res, obj, arg, order, Type, ReprType)         \
  do {                                                                         \
    zig_##Type zig_atomicrmw_expected;                                         \
    zig_atomic_load(zig_atomicrmw_expected, obj, zig_memory_order_relaxed,     \
                    Type, ReprType);                                           \
    while (!zig_cmpxchg_weak(obj, zig_atomicrmw_expected, arg, order,          \
                             zig_memory_order_relaxed, Type, ReprType))        \
      ;                                                                        \
...
#define zig_atomicrmw_add_float(res, obj, arg, order, Type, ReprType)          \
  do {                                                                         \
    ...
    } while (!zig_cmpxchg_weak(obj, zig_atomicrmw_expected,                    \
                               zig_atomicrmw_desired, order,                   \
                               zig_memory_order_relaxed, Type, ReprType));     \
    res = zig_atomicrmw_expected;                                              \
  } while (0)
...
#define zig_atomicrmw_nand_int128(res, obj, arg, order, Type, ReprType)        \
  do {                                                                         \
    ...
    zig_atomic_load(zig_atomicrmw_expected, obj, zig_memory_order_relaxed,     \
                    Type, ReprType);                                           \
    do {                                                                       \
      zig_atomicrmw_desired =                                                  \
          zig_not_##Type(zig_and_##Type(zig_atomicrmw_expected, arg), 128);    \
    } while (!zig_cmpxchg_weak(obj, zig_atomicrmw_expected,                    \
                               zig_atomicrmw_desired, order,                   \
                               zig_memory_order_relaxed, Type, ReprType));     \
    res = zig_atomicrmw_expected;                                              \
...
#define zig_atomicrmw_max_int128(res, obj, arg, order, Type, ReprType)         \
  do {                                                                         \
    ...
    (zig_has_include(<stdatomic.h>) && !defined(zig_msvc))
...
#define zig_cmpxchg_strong(obj, expected, desired, succ, fail, Type, ReprType) \
  atomic_compare_exchange_strong_explicit(obj, &(expected), desired, succ, fail)
#define zig_cmpxchg_weak(obj, expected, desired, succ, fail, Type, ReprType)   \
  atomic_compare_exchange_weak_explicit(obj, &(expected), desired, succ, fail)
#define zig_atomicrmw_xchg(res, obj, arg, order, Type, ReprType)               \
  res = atomic_exchange_explicit(obj, arg, order)
#define zig_atomicrmw_add(res, obj, arg, order, Type, ReprType)                \
  res = atomic_fetch_add_explicit(obj, arg, order)
#define zig_atomicrmw_sub(res, obj, arg, order, Type, ReprType)                \
  res = atomic_fetch_sub_explicit(obj, arg, order)
#define zig_atomicrmw_or(res, obj, arg, order, Type, ReprType)                 \
  res = atomic_fetch_or_explicit(obj, arg, order)
#define zig_atomicrmw_xor(res, obj, arg, order, Type, ReprType)                \
  res = atomic_fetch_xor_explicit(obj, arg, order)
#define zig_atomicrmw_and(res, obj, arg, order, Type, ReprType)                \
  res = atomic_fetch_and_explicit(obj, arg, order)
#define zig_atomicrmw_nand(res, obj, arg, order, Type, ReprType)               \
  res = __atomic_fetch_nand(obj, arg, order)
#define zig_atomicrmw_min(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_min(obj, arg, order)
#define zig_atomicrmw_max(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_max(obj, arg, order)
#define zig_atomic_store(obj, arg, order, Type, ReprType)                      \
  atomic_store_explicit(obj, arg, order)
#define zig_atomic_load(res, obj, order, Type, ReprType)                       \
...
#define zig_cmpxchg_strong(obj, expected, desired, succ, fail, Type, ReprType) \
  __atomic_compare_exchange(obj, (ReprType *)&(expected),                      \
                            (ReprType *)&(desired), false, succ, fail)
#define zig_cmpxchg_weak(obj, expected, desired, succ, fail, Type, ReprType)   \
  __atomic_compare_exchange(obj, (ReprType *)&(expected),                      \
                            (ReprType *)&(desired), true, succ, fail)
#define zig_atomicrmw_xchg(res, obj, arg, order, Type, ReprType)               \
  __atomic_exchange(obj, (ReprType *)&(arg), &(res), order)
#define zig_atomicrmw_add(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_add(obj, arg, order)
#define zig_atomicrmw_sub(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_sub(obj, arg, order)
#define zig_atomicrmw_or(res, obj, arg, order, Type, ReprType)                 \
  res = __atomic_fetch_or(obj, arg, order)
#define zig_atomicrmw_xor(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_xor(obj, arg, order)
#define zig_atomicrmw_and(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_and(obj, arg, order)
#define zig_atomicrmw_nand(res, obj, arg, order, Type, ReprType)               \
  res = __atomic_fetch_nand(obj, arg, order)
#define zig_atomicrmw_min(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_min(obj, arg, order)
#define zig_atomicrmw_max(res, obj, arg, order, Type, ReprType)                \
  res = __atomic_fetch_max(obj, arg, order)
#define zig_atomic_store(obj, arg, order, Type, ReprType)                      \
  __atomic_store(obj, (ReprType *)&(arg), order)
#define zig_atomic_load(res, obj, order, Type, ReprType)                       \
  __atomic_load(obj, &(res), order)
#undef zig_atomicrmw_xchg_float
#define zig_atomicrmw_xchg_float zig_atomicrmw_xchg
...
#define zig_atomic(Type) Type
#define zig_cmpxchg_strong(obj, expected, desired, succ, fail, Type, ReprType) \
  zig_msvc_cmpxchg_##Type(obj, &(expected), desired)
#define zig_cmpxchg_weak(obj, expected, desired, succ, fail, Type, ReprType)   \
  zig_cmpxchg_strong(obj, expected, desired, succ, fail, Type, ReprType)
#define zig_atomicrmw_xchg(res, obj, arg, order, Type, ReprType)               \
  res = zig_msvc_atomicrmw_xchg_##Type(obj, arg)
#define zig_atomicrmw_add(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_add_##Type(obj, arg)
#define zig_atomicrmw_sub(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_sub_##Type(obj, arg)
#define zig_atomicrmw_or(res, obj, arg, order, Type, ReprType)                 \
  res = zig_msvc_atomicrmw_or_##Type(obj, arg)
#define zig_atomicrmw_xor(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_xor_##Type(obj, arg)
#define zig_atomicrmw_and(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_and_##Type(obj, arg)
#define zig_atomicrmw_nand(res, obj, arg, order, Type, ReprType)               \
  res = zig_msvc_atomicrmw_nand_##Type(obj, arg)
#define zig_atomicrmw_min(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_min_##Type(obj, arg)
#define zig_atomicrmw_max(res, obj, arg, order, Type, ReprType)                \
  res = zig_msvc_atomicrmw_max_##Type(obj, arg)
#define zig_atomic_store(obj, arg, order, Type, ReprType)                      \
  zig_msvc_atomic_store_##Type(obj, arg)
#define zig_atomic_load(res, obj, order, Type, ReprType)                       \
  res = zig_msvc_atomic_load_##order##_##Type(obj)
...
/* TODO: zig_msvc_atomic_load should load 32 bit without interlocked on x86, and
 * load 64 bit without interlocked on x64 */

#define zig_msvc_atomics(ZigType, Type, SigType, suffix, iso_suffix)           \
  static inline bool zig_msvc_cmpxchg_##ZigType(                               \
      Type volatile *obj, Type *expected, Type desired) {                      \
    Type comparand = *expected;                                                \
    Type initial = _InterlockedCompareExchange##suffix(                        \
        (SigType volatile *)obj, (SigType)desired, (SigType)comparand);        \
  ...
  static inline Type zig_msvc_atomicrmw_xchg_##ZigType(Type volatile *obj,     \
                                                       Type value) {           \
    return _InterlockedExchange##suffix((SigType volatile *)obj,               \
                                        (SigType)value);                       \
  }                                                                            \
  static inline Type zig_msvc_atomicrmw_add_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    return _InterlockedExchangeAdd##suffix((SigType volatile *)obj,            \
                                           (SigType)value);                    \
  }                                                                            \
  static inline Type zig_msvc_atomicrmw_sub_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    bool success = false;                                                      \
    Type new;                                                                  \
    Type prev;                                                                 \
  ...
  static inline Type zig_msvc_atomicrmw_or_##ZigType(Type volatile *obj,       \
                                                     Type value) {             \
    return _InterlockedOr##suffix((SigType volatile *)obj, (SigType)value);    \
  }                                                                            \
  static inline Type zig_msvc_atomicrmw_xor_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    return _InterlockedXor##suffix((SigType volatile *)obj, (SigType)value);   \
  }                                                                            \
  static inline Type zig_msvc_atomicrmw_and_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    return _InterlockedAnd##suffix((SigType volatile *)obj, (SigType)value);   \
  }                                                                            \
  static inline Type zig_msvc_atomicrmw_nand_##ZigType(Type volatile *obj,     \
                                                       Type value) {           \
    bool success = false;                                                      \
    Type new;                                                                  \
    Type prev;                                                                 \
  ...
  static inline Type zig_msvc_atomicrmw_min_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    bool success = false;                                                      \
    Type new;                                                                  \
    Type prev;                                                                 \
  ...
  static inline Type zig_msvc_atomicrmw_max_##ZigType(Type volatile *obj,      \
                                                      Type value) {            \
    bool success = false;                                                      \
    Type new;                                                                  \
    Type prev;                                                                 \
  ...
  static inline void zig_msvc_atomic_store_##ZigType(Type volatile *obj,       \
                                                     Type value) {             \
    (void)_InterlockedExchange##suffix((SigType volatile *)obj,                \
                                       (SigType)value);                        \
  }                                                                            \
  static inline Type zig_msvc_atomic_load_zig_memory_order_relaxed_##ZigType(  \
      Type volatile *obj) {                                                    \
    return __iso_volatile_load##iso_suffix((SigType volatile *)obj);           \
  }                                                                            \
  static inline Type zig_msvc_atomic_load_zig_memory_order_acquire_##ZigType(  \
      Type volatile *obj) {                                                    \
    Type val = __iso_volatile_load##iso_suffix((SigType volatile *)obj);       \
    _ReadWriteBarrier();                                                       \
    return val;                                                                \
  }                                                                            \
  static inline Type zig_msvc_atomic_load_zig_memory_order_seq_cst_##ZigType(  \
      Type volatile *obj) {                                                    \
    Type val = __iso_volatile_load##iso_suffix((SigType volatile *)obj);       \
    _ReadWriteBarrier();                                                       \
    return val;                                                                \
...
zig_msvc_atomics(u8, uint8_t, char, 8, 8) zig_msvc_atomics(i8, int8_t, char, 8,
                                                           8)
    zig_msvc_atomics(u16, uint16_t, short, 16,
                     16) zig_msvc_atomics(i16, int16_t, short, 16, 16)
...
#define zig_msvc_flt_atomics(Type, SigType, suffix, iso_suffix)                \
  static inline bool zig_msvc_cmpxchg_##Type(                                  \
      zig_##Type volatile *obj, zig_##Type *expected, zig_##Type desired) {    \
    SigType exchange;                                                          \
    SigType comparand;                                                         \
    SigType initial;                                                           \
    bool success;                                                              \
    ...
    initial = _InterlockedCompareExchange##suffix((SigType volatile *)obj,     \
                                                  exchange, comparand);        \
  ...
  static inline void zig_msvc_atomic_store_##Type(zig_##Type volatile *obj,    \
                                                  zig_##Type arg) {            \
    SigType value;                                                             \
    memcpy(&value, &arg, sizeof(value));                                       \
    (void)_InterlockedExchange##suffix((SigType volatile *)obj, value);        \
  }                                                                            \
  static inline zig_##Type                                                     \
  zig_msvc_atomic_load_zig_memory_order_relaxed_##Type(                        \
      zig_##Type volatile *obj) {                                              \
    zig_##Type result;                                                         \
    SigType initial =                                                          \
        __iso_volatile_load##iso_suffix((SigType volatile *)obj);              \
    memcpy(&result, &initial, sizeof(result));                                 \
    return result;                                                             \
  }                                                                            \
  static inline zig_##Type                                                     \
  zig_msvc_atomic_load_zig_memory_order_acquire_##Type(                        \
      zig_##Type volatile *obj) {                                              \
    zig_##Type result;                                                         \
    SigType initial =                                                          \
        __iso_volatile_load##iso_suffix((SigType volatile *)obj);              \
  ...
  static inline zig_##Type                                                     \
  zig_msvc_atomic_load_zig_memory_order_seq_cst_##Type(                        \
      zig_##Type volatile *obj) {                                              \
    zig_##Type result;                                                         \
    SigType initial =                                                          \
        __iso_volatile_load##iso_suffix((SigType volatile *)obj);              \
...
static inline void *
zig_msvc_atomic_load_zig_memory_order_relaxed_p32(void volatile *obj) {
  return (void *)__iso_volatile_load32(obj);
}

static inline void *
zig_msvc_atomic_load_zig_memory_order_acquire_p32(void volatile *obj) {
...
static inline void *
zig_msvc_atomic_load_zig_memory_order_seq_cst_p32(void volatile *obj) {
  return zig_msvc_atomic_load_zig_memory_order_acquire_p32(obj);
}

static inline bool zig_msvc_cmpxchg_p32(void volatile *obj, void *expected,
                                        void *desired) {
...
#else /* zig_x86_32 */
                    static inline void *zig_msvc_atomicrmw_xchg_p64(
                        void volatile *obj, void *arg) {
  return _InterlockedExchangePointer(obj, arg);
}
...
static inline void *
zig_msvc_atomic_load_zig_memory_order_relaxed_p64(void volatile *obj) {
  return (void *)__iso_volatile_load64(obj);
}

static inline void *
zig_msvc_atomic_load_zig_memory_order_acquire_p64(void volatile *obj) {
...
static inline void *
zig_msvc_atomic_load_zig_memory_order_seq_cst_p64(void volatile *obj) {
  return zig_msvc_atomic_load_zig_memory_order_acquire_p64(obj);
}

static inline bool zig_msvc_cmpxchg_p64(void volatile *obj, void *expected,
                                        void *desired) {
  void *comparand = *(void **)expected;
  void *initial = _InterlockedCompareExchangePointer(obj, desired, comparand);
  bool success = initial == comparand;
...
static inline bool zig_msvc_cmpxchg_u128(zig_u128 volatile *obj,
                                         zig_u128 *expected, zig_u128 desired) {
  return _InterlockedCompareExchange128(
      (__int64 volatile *)obj, (__int64)zig_hi_u128(desired),
      (__int64)zig_lo_u128(desired), (__int64 *)expected);
}
...
static inline zig_u128 zig_msvc_atomic_load_u128(zig_u128 volatile *obj) {
  ...
  (void)zig_cmpxchg_strong(obj, expected, expected, zig_memory_order_seq_cst,
                           zig_memory_order_seq_cst, u128, zig_u128);
  return expected;
}

static inline void zig_msvc_atomic_store_u128(zig_u128 volatile *obj,
                                              zig_u128 arg) {
  zig_u128 expected = zig_make_u128(UINT64_C(0), UINT64_C(0));
  while (!zig_cmpxchg_weak(obj, expected, arg, zig_memory_order_seq_cst,
                           zig_memory_order_seq_cst, u128, zig_u128))
    ;
}

static inline bool zig_msvc_cmpxchg_i128(zig_i128 volatile *obj,
                                         zig_i128 *expected, zig_i128 desired) {
  return _InterlockedCompareExchange128(
      (__int64 volatile *)obj, (__int64)zig_hi_i128(desired),
      (__int64)zig_lo_i128(desired), (__int64 *)expected);
}
...
static inline zig_i128 zig_msvc_atomic_load_i128(zig_i128 volatile *obj) {
  ...
  (void)zig_cmpxchg_strong(obj, expected, expected, zig_memory_order_seq_cst,
                           zig_memory_order_seq_cst, i128, zig_i128);
  return expected;
}

static inline void zig_msvc_atomic_store_i128(zig_i128 volatile *obj,
                                              zig_i128 arg) {
  zig_i128 expected = zig_make_i128(INT64_C(0), UINT64_C(0));
  while (!zig_cmpxchg_weak(obj, expected, arg, zig_memory_order_seq_cst,
                           zig_memory_order_seq_cst, i128, zig_i128))
...
static inline void zig_x86_cpuid(uint32_t leaf_id, uint32_t subid,
