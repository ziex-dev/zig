pub fn addCases(cases: *tests.LibcContext) void {
    ...
    // TODO: Find out why atan2 fails on thumb, thumbeb and ppc. It seems to be a flush-to-zero issue.
    cases.addLibcTestCase("math/atan2.c", true, .{ .skip_arches = &.{ .arm, .armeb, .thumb, .thumbeb, .powerpc } });
    cases.addLibcTestCase("math/atan2f.c", true, .{ .skip_arches = &.{ .arm, .armeb, .thumb, .thumbeb, .powerpc } });
    cases.addLibcTestCase("math/atan2l.c", true, .{ .skip_arches = &.{ .arm, .armeb, .thumb, .thumbeb, .powerpc } });
